...
static const WORD8 InvSbox[256] = { 0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5,
...
static const WORD32 rcon[10] = { 0x01000000, 0x02000000, 0x04000000,
		0x08000000, 0x10000000, 0x20000000, 0x40000000, 0x80000000/*x alla 7*/,
...
void
		encryptblock(WORD32* input_pointer, WORD32* output_pointer,
				WORD32* expkey);
void
		decryptblock(WORD32* input_pointer, WORD32* output_pointer,
				WORD32* expkey);
...
void keyschedule(WORD32* key, WORD32* exp) {
	...
	((key[1] & 0xff000000) >> 8) | ((key[2] & 0xff000000) >> 16) | ((key[3]
			& 0xff000000) >> 24);
	copy1 = ((key[0] & 0x00ff0000) << 8) | (key[1] & 0x00ff0000) | ((key[2]
			& 0x00ff0000) >> 8) | ((key[3] & 0x00ff0000) >> 16);
	copy2 = ((key[0] & 0x0000ff00) << 16) | ((key[1] & 0x0000ff00) << 8)
			| (key[2] & 0x0000ff00) | ((key[3] & 0x0000ff00) >> 8);
	copy3 = ((key[0] & 0x000000ff) << 24) | ((key[1] & 0x000000ff) << 16)
			| ((key[2] & 0x000000ff) << 8) | (key[3] & 0x000000ff);/*Fine trasposizione matrice chiave*/
...
void encryptblock(WORD32* input_pointer, WORD32* output_pointer, WORD32* expkey) {
	...
	s0 = ((input_pointer[0] & 0xff000000) | ((input_pointer[1] & 0xff000000)
			>> 8) | ((input_pointer[2] & 0xff000000) >> 16)
			| ((input_pointer[3] & 0xff000000) >> 24)) ^ local_pointer[0];
	s1 = (((input_pointer[0] & 0x00ff0000) << 8) | (input_pointer[1]
			& 0x00ff0000) | ((input_pointer[2] & 0x00ff0000) >> 8)
			| ((input_pointer[3] & 0x00ff0000) >> 16)) ^ local_pointer[1];
	s2 = (((input_pointer[0] & 0x0000ff00) << 16) | ((input_pointer[1]
			& 0x0000ff00) << 8) | (input_pointer[2] & 0x0000ff00)
			| ((input_pointer[3] & 0x0000ff00) >> 8)) ^ local_pointer[2];
	s3 = (((input_pointer[0] & 0x000000ff) << 24) | ((input_pointer[1]
			& 0x000000ff) << 16) | ((input_pointer[2] & 0x000000ff) << 8)
	...
	output_pointer[0] = (t0 & 0xff000000) | ((t1 & 0xff000000) >> 8) | ((t2
			& 0xff000000) >> 16) | ((t3 & 0xff000000) >> 24);
	output_pointer[1] = ((t0 & 0x00ff0000) << 8) | (t1 & 0x00ff0000) | ((t2
			& 0x00ff0000) >> 8) | ((t3 & 0x00ff0000) >> 16);
	output_pointer[2] = ((t0 & 0x0000ff00) << 16) | ((t1 & 0x0000ff00) << 8)
			| (t2 & 0x0000ff00) | ((t3 & 0x0000ff00) >> 8);
	output_pointer[3] = ((t0 & 0x000000ff) << 24) | ((t1 & 0x000000ff) << 16)
			| ((t2 & 0x000000ff) << 8) | (t3 & 0x000000ff);
...
int main() {
	...
	//
#ifdef _LINUX_
	printf("Benchmark: AES\n");
	printf("%s\n",__DATE__);
#else
	print_uart("Benchmark: AES\n");
	print_uart(__DATE__);
	print_uart("\n");
#endif
	...
	//asm volatile ("rdcycle %0" : "=r" (cycles_a));
	//print_uart("b\n");
	...
	WORD32 key[4]    = { 0x2b7e1516, 0x28aed2a6, 0xabf71588, 0x09cf4f3c };
	WORD8 text[]     = "ABCDEFGHIJKLMNOP";
	WORD8 text2[]    = "                ";
	WORD32 *block    = (WORD32*) text;
	WORD32 *outdecry = (WORD32*) text2;
	WORD32 outencry[4];

	keyschedule(key,       m_expanded_key);
	encryptblock(block,    outencry, m_expanded_key);
	decryptblock(outencry, outdecry, m_expanded_key);

  //
  // TIME MEASURE + OUTPUT TIMING
  //

//	asm volatile ("rdcycle %0" : "=r" (cycles_b));
//#ifdef _LINUX_
//	printf("Total cycles: %lu\n",cycles_b-cycles_a);
//#else
//	print_uart("Total cycles: ");
//	print_uart_int(cycles_b-cycles_a);
//	print_uart("\n");
//#endif
